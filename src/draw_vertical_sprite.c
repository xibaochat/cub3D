...
/*   Updated: 2020/04/20 18:24:00 by osshit           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_tex(t_spr *indi_spr)
{
	return ((int)((256 * (indi_spr->stripe -
...
void		draw_vertical_sprite(t_spr *indi_spr, t_map *rvar)
{
	...
	while (indi_spr->stripe++ < indi_spr->draw_end_x - 1)
	{
			...
			&& indi_spr->transform_y < rvar->spr_buffer[indi_spr->stripe])
		{
			...
			{
				rvar->d = y * 256 - rvar->height * 128\
				+ indi_spr->sprite_height * 128;
